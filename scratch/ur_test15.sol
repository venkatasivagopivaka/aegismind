// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;
import "forge-std/Script.sol";

contract URTest {
    function dispatch(bytes calldata inputs) external pure returns (uint256 io, uint256 off, uint256 ao, uint256 l, uint256 il) {
        assembly {
            io := inputs.offset
            off := calldataload(add(inputs.offset, 0x60))
            ao := add(inputs.offset, off)
            l := calldataload(ao)
            il := inputs.length
        }
    }
}

contract Runner is Script {
    function run() public {
        URTest test = new URTest();
        bytes memory data = hex"";
        (uint256 io, uint256 off, uint256 ao, uint256 l, uint256 il) = test.dispatch(data);
        console.log("inputs.offset:", io);
        console.log("offset:", off);
        console.log("arrayOffset:", ao);
        console.log("length:", l);
        console.log("inputs.length:", il);
    }
}
