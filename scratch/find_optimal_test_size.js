...
async function main() {
  ...
  const amounts = [0.1, 0.25, 0.5, 1, 2, 3, 5, 10, 15, 19];
  
  console.log("amount USDC | quote WETH | Chainlink floor | Hook accepts? | account funded? | likely executable?");
  console.log("-------------------------------------------------------------------------------------------------");
  
  for (const amt of amounts) {
    const amountIn = BigInt(Math.floor(amt * 1000000));
    
    // exact AegisMindHook calculation
    ...
    // minOut passed by setupKernel.ts is 99% of quote
    ...
    console.log(`${amt.toString().padEnd(11)} | ${quote.toString().padEnd(10)} | ${calculatedMin.toString().padEnd(15)} | ${hookAccepts.toString().padEnd(13)} | ${accountFunded.toString().padEnd(15)} | ${likelyExecutable}`);
  }
}
main();
